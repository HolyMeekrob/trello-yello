...
const TrelloObj = function (objType, config, id, net, initialVals) {
	...
	const setSubProperty = (value, path) => {
		...
		const parent = R.reduce(R.flip(R.propOr({})), trelloPropertyNames,
				R.insert(1, 'put', searchPath));
		const child = R.last(path);

		// If the parent is an empty object it means that the given property chain
		// is not in the Trello Property maps tree
		const parentDoesNotExist = R.isEmpty(R.keys(parent));
		...
		// The child is neither in the property chain nor does the parent allow IDs.
		// Since IDs can be anything, if the parent does allow IDs, we have to
		// accept the child no matter its value.
		...
		// If the child cannot be set through the Trello API
